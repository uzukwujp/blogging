...
it('returns 401 when you try to delete a comment that is not yours', async ()=>{
...
it('returns 200 on sucessfuly deleting a comment', async()=>{
    const postId = new mongoose.Types.ObjectId().toHexString()
    const cookie = global.getCookie('lkth', 'test@test.com')
...
it('publishes an event when deleting a comment', async ()=>{
  const postId = new mongoose.Types.ObjectId().toHexString()
    const cookie = global.getCookie('lkth', 'test@test.com')
